...
const app = createApp({
    ...
    methods: {
        ...
        drawWheel: function () {
            ...
            // G‘ildirak tashqi halqasini chizish
            ...
            // >>> Optimized left-pointing pointer (g‘ildirakning o‘ng tomonida joylashgan)
            const pointerX = this.centerX + this.outerRadius + 5; // Closer to wheel
            const pointerY = this.centerY;
            const pointerLength = 25; // Longer, sleeker arrow
            const pointerWidth = 10; // Narrower base

            this.ctx.beginPath();
            this.ctx.moveTo(pointerX, pointerY); // Tip (points left)
            this.ctx.lineTo(pointerX + pointerLength, pointerY - pointerWidth); // Top of base
            this.ctx.quadraticCurveTo(
                pointerX + pointerLength * 0.8, pointerY,
                pointerX + pointerLength, pointerY + pointerWidth
            ); // Smooth curve to bottom
            this.ctx.closePath();

            // Gradient
            const pointerGradient = this.ctx.createLinearGradient(pointerX, pointerY, pointerX + pointerLength, pointerY);
            pointerGradient.addColorStop(0, '#4CAF50');
            pointerGradient.addColorStop(1, '#66BB6A'); // Brighter green
